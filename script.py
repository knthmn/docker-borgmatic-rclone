...
try:
    if task == 'create':
        ...
        subprocess.run('borgmatic -v 1', shell=True, check=True)
        subprocess.run(f'rclone sync /mnt/repo {rclone_destination} --stats-log-level NOTICE --stats-one-line {rclone_args}', shell=True, check=True)
    elif task == 'check':
